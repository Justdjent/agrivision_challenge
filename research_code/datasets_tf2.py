...
from research_code.random_transform_mask import ImageWithMaskFunction, pad_img, tiles_with_overlap, read_img_opencv, \
    rgb2rgg, get_window
...
class DataGenerator_v2(tf.keras.utils.Sequence):
    ...
    def read_masks_borders(self, name, input_shape):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for ind, row in train_batch.iterrows():
            img_path = os.path.join(self.img_dir, 'images', "rgb", row['name'])
            img = cv2.imread(img_path)
            try:
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            except Exception as error:
                print(img_path)
                print(error)
            not_valid_mask = self.read_masks_borders(row['name'], img.shape)
            img[not_valid_mask] = 0
            img = cv2.resize(img, self.out_size)
            label_stack = np.zeros((len(self.classes), img.shape[0], img.shape[1]))
            for idx, cls in enumerate(self.classes):
                mask_path = os.path.join(self.img_dir, 'labels', cls, row['name'])
                mask = cv2.imread(mask_path.replace(".jpg", ".png"), cv2.IMREAD_GRAYSCALE)
                mask[not_valid_mask[:, :, 0]] = 0
                mask = mask > 0
                label_stack[idx] = mask
            masks.append(np.moveaxis(label_stack, 0, -1))
            batch_x.append(img)
        ...
        if self.activation == 'softmax':
            ...
            for idx, cls in enumerate(self.classes):
                masks[:, :, :, idx] *= class_priorities[cls]
            # (height, width, classes) -> (height, width)
            highest_score_label = masks.argmax(axis=-1)
            # (height, width) -> (height, width, classes)

            masks = tf.one_hot(highest_score_label, len(self.classes), dtype=np.float32).numpy()

        # mock cropping
        batch_x = batch_x[:, :224, :224, :]
        masks = masks[:, :224, :224, :]
        # augmentations to be added
        # batch_x, masks = self.mask_function.mask_fpred_angles(
        #     batch_x,
        #     class_labels,
        #     self.classes,
        #     self.aug,
    ...
    while True:
        ...
        for start in range(0, len(filenames), batch_size):
            ...
            for ind, filename in train_batch.iterrows():
                ...
                mask_load_path = os.path.join(img_man_dir, str(filename['folder']), 'np_arrays',
                                              filename['name'].replace(".png", ".npy"))
