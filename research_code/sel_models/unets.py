...
from research_code.sel_models.xception_padding import Xception
from research_code.sel_models.resnets import ResNet101, ResNet152, ResNet50
from research_code.sel_models.resnetv2 import InceptionResNetV2Same
...
def conv_bn_relu(input, num_channel, kernel_size, stride, name, padding='same', bn_axis=-1, bn_momentum=0.99,
                 bn_scale=True, use_bias=True, coord_conv=args.coord_conv):
    if coord_conv:
...
def conv_relu(input, num_channel, kernel_size, stride, name, padding='same', use_bias=True, activation='relu', coord_conv=args.coord_conv):
    if coord_conv:
        input = CoordinateChannel2D()(input)
    x = Conv2D(filters=num_channel, kernel_size=(kernel_size, kernel_size),
               strides=stride, padding=padding,
