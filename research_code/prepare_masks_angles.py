...
import shutil
from utils import polygons_to_image_view, points_to_image_view, mask_raster_with_polygon
POLYGONIZATION_THRESHOLD = 0.8
...
def run_mask_row_generaion(raster_path, polygons_path, window_size_meters, save_path, row_path, grove_bound=None):
    images_path = os.path.join(save_path, IMAGES_FOLDER)
    ...
    with rasterio.open(raster_path, "r") as dataset:
        ...
        for num, window in enumerate(windows_coordinates):
            ...
            if len(window_cut_polygons.index) > 0:
                ...
                segments = polygons_to_image_view(window_cut_polygons, rect=(window[2], window[0]), dataset=dataset)
                row_segments = points_to_image_view(window_cut_rows, rect=(window[2], window[0]), dataset=dataset)
                mask = np.zeros((res.shape[1], res.shape[2]), dtype=np.uint8)
...
if __name__ == "__main__":
    ...
    run_mask_row_generaion(args.raster_path, args.polygons_path, args.window_size_meters, args.save_path, args.row_path,
