...
class Reader:
    ...
    def create_rgb(self, int_float=False):
        if not ('rgbn' in self.raster_list or 'rgb' in self.raster_list):
            path = self.raster_list['green'].split('/')
            path[-1] = path[-1].replace('green', 'rgb')
            self.raster_list['rgb'] = "/".join(path)
            self.meta = {}
            pathes = self.raster_list
            with rasterio.open(self.raster_list['green'], 'r') as src:
                meta = src.meta
                ...
                if int_float:
                    ...
                    meta['nodata'] = 0
                # print(save_path)
            with rasterio.open(self.raster_list['rgb'], 'w', **meta) as dst:
                for i, raster in enumerate(['red', 'green', 'blue']):
                    with rasterio.open(pathes[raster], 'r') as src:
                        if int_float:
                            arr = src.read(1).astype('uint8')
                        else:
            ...
            if 'rgb' in self.raster_list:
                ...
                path = self.raster_list['rgbn'].split('/')
                rp = self.raster_list['rgbn']
            path[-1] = path[-1].split('.')[0] + '_rgb.tif'
            self.raster_list['rgb'] = "/".join(path)
            self.meta = {}
            with rasterio.open(rp, 'r') as src:
                meta = src.meta
...
class SegmentatorNN:
    def __init__(self, inp_list, thresh, r_type):
        ...
        #if 'rgb' in inp.keys():
        # if self.r_type == 'rgb' and 'rgb' not in inp.keys():
        #     self.reader.create_rgb()
        # else:
    ...
    def mask_tiles(self, save_mask=True, window_size=60):
        ...
        with rasterio.open(self.reader.raster_list[self.r_type], 'r') as dataset:
            meta = dataset.meta
            raster_array = np.zeros((dataset.meta['height'], dataset.meta['width']), np.uint8)
            ...
            while xs < dataset.bounds.right:
                ...
                while ys < dataset.bounds.top:
                    ...
                    if not isinstance(cv_res, np.ndarray):
                        continue
                    cv_res = cv_res[:, :, :3]
                    cv_res = cv2.resize(cv_res, (args.input_width, args.input_width), cv2.INTER_NEAREST)
        ...
        save_raster_dir = os.path.join(raster_dir, "coord_conv_rows_1349")
...
### mask with nanmean
def health_pred_on_image(model, image, classes, window_h=224, window_w=224, step_size=224, ref_pad=16):
    class_masks = OrderedDict()
    for c in classes:
        class_masks[c] = np.full(image.shape[:-1], np.nan)
    h, w, _ = image.shape
    pos_w = 0
    pos_h = 0
    ...
    #     v_flip=True,
    #     merge='mean',
    #     input_shape=(window_h + ref_pad*2, window_w + ref_pad*2, 3)
    # )
    while pos_h < h:
        next_h = pos_h + window_h
        pos_w = 0
        while pos_w < w:
            next_w = pos_w + window_w
            tile = image[pos_h:next_h, pos_w:next_w, :]
            # Preprocessing
            tile, pads = pad(tile, (window_h, window_w))
            tile = cv2.copyMakeBorder(
                tile, ref_pad, ref_pad, ref_pad, ref_pad, cv2.BORDER_REFLECT_101
            )
            ...
            preds = preds[0]
            # Postprocessing
            preds = preds[ref_pad:-ref_pad, ref_pad:-ref_pad, :]
            preds = unpad(preds, pads)
            preds = np.transpose(preds, axes=[2,0,1])
            for i, c in enumerate(class_masks):
                p = preds[i]
                prev = class_masks[c][pos_h:next_h, pos_w:next_w]
                stacked = np.dstack([p, prev])
                merged = np.nanmean(stacked, axis=2)
                class_masks[c][pos_h:next_h, pos_w:next_w] = merged
            pos_w += step_size
        pos_h += step_size
    return class_masks
